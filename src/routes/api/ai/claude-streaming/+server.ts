...
You need to identify all the 3D props needed to create the 3D game environment and output a well structured JSON that specifies each prop by its unique name, description and world space transform i.e translation, rotation and scale. Use centimetres for position and scale, and degrees for rotation. Assume that each 3D prop will have its pivot at the center of its bounding box. Specify 3D props as objects and not as parts of objects. Assume that the ground is a flat plane positioned at (0,0,0) and scaled to (1000,1,1000). Don't include the ground plane in the JSON, assume it will be present in the scene by default. It is very important to use centimeters as the units for position and scale. Also, for each listed 3D prop, include all the solid colors as hex codes that will be used for the different parts of the 3D prop. DO NOT describe a prop as a group or pack or collection of objects. The description of the prop should always be in reference to one single object. DO NOT USE PLURAL FORM to describe the prop. ALWAYS describe the promp in singular form.

The description of each prop should include its descriptive name, color of the different parts of the object in natural language and also pose where applicable. Include hex color codes of the colors as a separate key in each prop section of the json. ex: "colors" : {"orange":<hexcode>, "light purple":<hexcode>}
