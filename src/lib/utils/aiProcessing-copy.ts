 ...
 */
export async function callGenerateAndUploadModelApi(payload: ModelGenPayload): Promise<ApiResponse<{ r2Key: string; publicUrl: string }>> {
...
export async function processSceneData(
    ...
    for (let i = 0; i < modelQueue.length; i++) {
        ...
        let modelResult: ApiResponse<{ r2Key: string; publicUrl: string }>;
        let modelRetries = 0;
        ...
        if (!modelResult!.success) {
            ...
            finalResult.modelR2Key = modelResult!.data!.r2Key;
            finalResult.modelPublicUrl = modelResult!.data!.publicUrl;
